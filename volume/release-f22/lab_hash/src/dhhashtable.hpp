...
template <class K, class V> DHHashTable<K, V>::DHHashTable(size_t tsize) {
  if (tsize <= 0)
    tsize = 17;
...
template <class K, class V> DHHashTable<K, V>::~DHHashTable() { destroy(); }
...
template <class K, class V> void DHHashTable<K, V>::destroy() {
  for (size_t i = 0; i < size; i++)
    delete table[i];
  delete[] table;
  table = nullptr;
...
void DHHashTable<K, V>::insert(K const &key, V const &value) {
  ...
  table[idx] = new std::pair(key, value);
  should_probe[idx] = true;
}

template <class K, class V> void DHHashTable<K, V>::remove(K const &key) {
  /**
   * @todo Implement this function
   */
...
template <class K, class V> V DHHashTable<K, V>::find(K const &key) const {
  int idx = findIndex(key);
  if (idx != -1)
...
template <class K, class V> V &DHHashTable<K, V>::operator[](K const &key) {
...
template <class K, class V> void DHHashTable<K, V>::clear() {
  destroy();
...
template <class K, class V> void DHHashTable<K, V>::resizeTable() {

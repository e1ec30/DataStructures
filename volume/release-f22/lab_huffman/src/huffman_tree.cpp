...
void HuffmanTree::buildTree(const vector<Frequency> &frequencies) {
  ...
  while ((singleQueue.size() + mergeQueue.size()) > 1) {
    ...
    TreeNode *internal =
        new TreeNode(left->freq.getFrequency() + right->freq.getFrequency());
    internal->left = left;
...
HuffmanTree::TreeNode *HuffmanTree::readTree(BinaryFileReader &bfile) {
  ...
  bool bit = bfile.getNextBit();
  if (bit) {
    Frequency tf = Frequency(bfile.getNextByte(), 0);
    TreeNode *n = new TreeNode(tf);
    return n;
  } else {
...
Frequency HuffmanTree::testRemoveSmallest(vector<Frequency> single,

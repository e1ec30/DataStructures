...
size_t heap<T, Compare>::maxPriorityChild(size_t currentIdx) const {
  ...
  if (right_child >= _elems.size()) return left_child;

  else if (higherPriority(_elems[left_child], _elems[right_child])) {
    return left_child;
  } else {
    return right_child;
  }
...
heap<T, Compare>::heap(const std::vector<T> &elems) {
  ...
  for (ssize_t i = parent(_elems.size() - 1); i >= 0; --i) {
      // print_vector(_elems);
      heapifyDown(i);
      // print_vector(_elems);
  }

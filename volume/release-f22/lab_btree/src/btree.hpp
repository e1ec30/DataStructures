...
void BTree<K, V>::split_child(BTreeNode *parent, size_t child_idx) {
  ...
  // e1ec30: first, add the middle element to the parent.
  parent->elements.insert(elem_itr, *mid_elem_itr);

  // e1ec30: then initialize new_right.
...
void BTree<K, V>::insert(BTreeNode *subroot, const DataPair &pair) {
  ...
  if (subroot->is_leaf) {
    ...
    // first_larget_idx will conveniently tell us which child to insert into.
    BTreeNode *insertion_child = subroot->children[first_larger_idx];
    insert(insertion_child, pair);
    if (insertion_child->elements.size() >= this->order)
